	...
	t_pipe *ctx, t_env *env, int redirect_error)
{
	...
	{
		/* Set up child process signal handling */
...
static void save_signal_handlers(struct sigaction *old_int, struct sigaction *old_quit)
{
...
static void set_parent_signals_for_pipeline(void)
{
	...
	sa.sa_handler = SIG_IGN;  /* Parent process ignores signals during pipeline */
	sa.sa_flags = 0;
...
static void restore_signal_handlers(struct sigaction *old_int, struct sigaction *old_quit)
{
...
void	execute_pipeline(t_cmd *cmd, t_env *env)
{
	...
	env->at_prompt = 1;
	// setup_signal(env); //add
	// tcsetpgrp(STDIN_FILENO, getpid()); //add
}
